...
class ModelCatalogProduct extends Model
{
    ...
    {
        $this->db->query("INSERT INTO " . DB_PREFIX . "product SET model = '" . $this->db->escape($data['model']) . "', sku = '" . $this->db->escape($data['sku']) . "', upc = '" . $this->db->escape($data['upc']) . "',jan = '" . $this->db->escape($data['jan']) . "', isbn = '" . $this->db->escape($data['isbn']) . "', mpn = '" . $this->db->escape($data['mpn']) . "', location = '" . $this->db->escape($data['location']) . "', quantity = '" . (int)$data['quantity'] . "', minimum = '" . (int)$data['minimum'] . "', subtract = '" . (int)$data['subtract'] . "', stock_status_id = '" . (int)$data['stock_status_id'] . "', date_available = '" . $this->db->escape($data['date_available']) . "', manufacturer_id = '" . (int)$data['manufacturer_id'] . "', shipping = '" . (int)$data['shipping'] . "', price = '" . (float)$data['price'] . "', points = '" . (int)$data['points'] . "', weight = '" . (float)$data['weight'] . "', weight_class_id = '" . (int)$data['weight_class_id'] . "', length = '" . (float)$data['length'] . "', width = '" . (float)$data['width'] . "', height = '" . (float)$data['height'] . "', length_class_id = '" . (int)$data['length_class_id'] . "', status = '" . (int)$data['status'] . "', tax_class_id = '" . (int)$data['tax_class_id'] . "', sort_order = '" . (int)$data['sort_order'] . "', date_added = NOW(), date_modified = NOW()");
        ...
        if (isset($data['product_taobao_id'])) {
    ...
    public function editProductTaobao($num_iid,$data){
        //print_r($data);
        $sql = "UPDATE " . DB_PREFIX . "product_to_taobao SET reserve_price = '" . (float)$data['reserve_price'] . "', zk_final_price = '" . (float)$data['zk_final_price'] . "', item_url = '" . $this->db->escape($data['item_url']) . "',user_type ='" . (int)$data['user_type'] . "' ,provcity = '" . $this->db->escape($data['provcity']) . "', click_url = '" . $this->db->escape($data['click_url']) . "', seller_id = '" . (int)$data['seller_id'] . "', volume = '" . (int)$data['volume'] . "', nick = '" . $this->db->escape($data['nick']) . "', shop_title = '" . $this->db->escape($data['shop_title']) . "', zk_final_price_wap = '" .(float)$data['zk_final_price_wap'] . "', event_start_time = '" . $this->db->escape($data['event_start_time']) . "', event_end_time = '" . $this->db->escape($data['event_end_time']) . "', tk_rate = '" . $this->db->escape($data['tk_rate']) . "', status = '" . (int)$data['status'] . "', type = '" . (int)$data['type'] . "'";
        if (isset($data['coupon_info'])) {
        ...
        $sql .= ' WHERE num_iid = "'.$this->db->escape($num_iid) . '"';
        echo '<br/>';
        echo '=================================';
        print_r($sql);
        echo '=================================';
    ...
    public function checkInRow($product_id)
    {
        $query = $this->db->query('SELECT COUNT(*) AS total FROM ' . DB_PREFIX . 'product_to_taobao WHERE num_iid="' . $product_id . '"');
        return $query->row['total'];
    }

    public function updateProductDesc($desc, $product_id)
    {
        $this->db->query('UPDATE ' . DB_PREFIX . 'product_description SET description="' . $this->db->escape($desc) . '" WHERE product_id="' . $product_id . '"');
    }

    public function getProductDesc()
    {
        $query = $this->db->query('SELECT product_id,item_url FROM ' . DB_PREFIX . 'product WHERE item_url IS NOT NULL');
        return $query->rows;
    }

    public function addProductFromTaobao($products)
    {
        if ($products) {
            //print_r($products);
            foreach ($products as $key => $pro) {
                foreach ($pro as $product) {
                    //print_r($product);
                    $data = array();
                    $data['model'] = $product['num_iid'];
                    $data['price'] = $product['zk_final_price'];
                    $data['status'] = 1;
                    $data['quantity'] = (int)$product['volume'] < 10 ? 9999 : $product['volume'];
                    $data['image'] = $product['pict_url'];
                    ...
                    if (isset($product['coupon_info'])){
                        ...
                        $data['coupon_minus'] = $product['coupon_minus'];
                        $data['coupon_click_url'] = $product['coupon_click_url'];
                        $data['coupon_end_time'] = $product['coupon_end_time'];
                        $data['coupon_start_time'] = $product['coupon_start_time'];
                        $data['coupon_remain_count'] = $product['coupon_remain_count'];
                        $data['coupon_total_count'] = $product['coupon_total_count'];
                    }
                    $data['product_taobao_id'] = $product['num_iid'];
                    $data['reserve_price'] = $product['reserve_price'];
                    $data['zk_final_price'] = $product['zk_final_price'];
                    $data['user_type'] = $product['user_type'];
                    $data['provcity'] = $product['provcity'];
                    $data['item_url'] = $product['item_url'];
                    $data['click_url'] = isset($product['click_url']) ? $product['click_url'] : $product['item_url'];
                    $data['seller_id'] = $product['seller_id'];
                    $data['volume'] = $product['volume'];
                    $data['nick'] = $product['nick'];
                    $data['shop_title'] = $product['shop_title'];
                    $data['zk_final_price_wap'] = $product['zk_final_price_wap'];
                    $data['event_start_time'] = $product['event_start_time'];
                    $data['event_end_time'] = $product['event_end_time'];
                    $data['tk_rate'] = $product['tk_rate'];
                    ...
                    if (isset($product['small_images'])) {
                        if(isset($product['small_images']['string'])) {
                            foreach ($product['small_images']['string'] as $image) {
                                $data['product_image'][] = array(
                                    'image' => $image,
                                    'sort_order' => 0,
                                );
                    ...
                    if ($this->checkInRow($product['num_iid']) == 0) {
                        //var_dump($this->checkTitleIsIn($product['num_iid']));
                        $this->addProduct($data);
                    }else{
                        $this->editProductTaobao($product['num_iid'],$data);
                    }
                }
            }
        }
