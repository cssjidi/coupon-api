...
class Customer {
  ...
  public function login($email, $password, $override = false) {
		if ($override) {
			...
			//Email login
			$customer_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "customer WHERE LOWER(email) = '" . $this->db->escape(utf8_strtolower($email)) . "' AND (password = SHA1(CONCAT(salt, SHA1(CONCAT(salt, SHA1('" . $this->db->escape($password) . "'))))) OR password = SHA1(CONCAT(salt, SHA1(CONCAT(salt, '" . $this->db->escape($password) . "')))) OR password = '" . $this->db->escape(md5($password)) . "') AND status = '1'");
			
			if ($customer_query->num_rows == 0) {
				//Telephone login
				$customer_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "customer WHERE LOWER(telephone) = '" . $this->db->escape(utf8_strtolower($email)) . "' AND (password = SHA1(CONCAT(salt, SHA1(CONCAT(salt, SHA1('" . $this->db->escape($password) . "'))))) OR password = SHA1(CONCAT(salt, SHA1(CONCAT(salt, '" . $this->db->escape($password) . "')))) OR password = '" . $this->db->escape(md5($password)) . "') AND status = '1'");
